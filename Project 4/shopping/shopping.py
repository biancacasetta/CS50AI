    ...
    with open(filename, newline='') as f:
        ...
        for row in reader:
            ...
            for column, value in row.items():
                match column:
                    case "Administrative" | "Informational" | "ProductRelated" | \
                            "OperatingSystems" | "Browser" | "Region" | "TrafficType":
                        e.append(int(value))
                    case "Administrative_Duration" | "Informational_Duration" | \
                            "ProductRelated_Duration" | "BounceRates" | "ExitRates" | \
                            "PageValues" | "SpecialDay":
                        e.append(float(value))
                    case "Month":
                        e.append(months[value])
                    case "VisitorType":
                        e.append(1 if value == "Returning_Visitor" else 0)
                    case "Weekend":
                        e.append(1 if value == "TRUE" else 0)
                    case "Revenue":
